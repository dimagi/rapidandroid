 ...
 * @created Jan 19, 2009
 * 
 *          Static methods to convert the Android Cursors to
 *          org.rapidsms.java.core.model types
 * 
 */
...
public class ModelConverter {
	...
	public static Form getFormFromUri(ContentProvider provider, Uri formUri) {
		...
		Cursor formCursor = provider.query(formUri, null, null, null, null); // hack
																				// way
		// Cursor formCursor = context.getContextResolver().query(formUri,
		// null,null,null,null); //real way
	...
	public static Field[] getFieldsForForm(ContentProvider provider, int formId) { // hack
		...
		do {
			...
			// Field newField = new
			// Field(id,sequence,name,prompt,getFieldType(context,fieldtype));
			// //real way
			Field newField = new Field(id, sequence, name, prompt,
					getFieldType(provider, fieldtype));// hack way
	...
	// public static FieldType getFieldType(Context context, int type_id) {
	// //real way
	public static FieldType getFieldType(ContentProvider provider, int type_id) { // hack
																					// way
		...
		Uri typeUri = Uri.parse(RapidSmsDataDefs.FieldType.CONTENT_URI_STRING
				+ type_id);
		// Cursor typeCursor = context.GetContentResolver().query(typeUri,
		// null,null, null, null); //real way
