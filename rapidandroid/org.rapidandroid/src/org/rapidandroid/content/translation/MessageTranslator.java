	...
	public static synchronized void updateMonitorHash(Context context) {
		...
		if (monitorCursor.moveToFirst()) {
			...
			do {
				...
				Monitor newMonitor = new Monitor(monitorCursor
						.getInt(Monitor.COL_ID), monitorCursor
						.getString(Monitor.COL_FIRSTNAME), monitorCursor
						.getString(Monitor.COL_LASTNAME), monitorCursor
						.getString(Monitor.COL_ALIAS), monitorCursor
						.getString(Monitor.COL_PHONE), monitorCursor
						.getString(Monitor.COL_EMAIL), monitorCursor
						.getInt(Monitor.COL_MESSAGECOUNT), monitorCursor
						.getInt(Monitor.COL_RECEIVE_REPLY) == 1);
	...
	public static synchronized Message GetMessage(Context context, int messageID) {
		...
		if (msgCursor.getCount() != 1) {
			...
			try {
				...
				Message newMessage = new Message(msgCursor
						.getInt(Message.COL_ID), msgCursor
						.getString(Message.COL_MESSAGE), msgDate, mMonitorHash
						.get(Integer.valueOf(msgCursor
